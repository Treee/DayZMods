class IAT_DynamicRadioConfig
{
	...
	IAT_DynamicRadioConfig TryGetDynamicRadioConfig()
	{
		...
		string errorMessage;
		IAT_DynamicRadioConfig iat_DRConfig;
		...
		if (!FileExist(jsonConfig))
		{
			...
			if (!JsonFileLoader<ref IAT_DynamicRadioConfig>.SaveFile(jsonConfig, iat_DRConfig, errorMessage))
				ErrorEx(errorMessage);
		}
		else
		{
			// file exists, just load it from disk
			if (!JsonFileLoader<ref IAT_DynamicRadioConfig>.LoadFile(jsonConfig, iat_DRConfig, errorMessage))
				ErrorEx(errorMessage);
		}
