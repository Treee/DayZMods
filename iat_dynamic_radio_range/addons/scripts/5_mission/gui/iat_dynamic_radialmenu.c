class IAT_DR_RadialMenuDisplay extends UIScriptedMenu
{
	protected Widget 							m_ItemCardPanel;
	protected ref array<ref IAT_DR_RadialMenuItem> 	m_RadialItems;
	...
	//instance
	static IAT_DR_RadialMenuDisplay							instance;

	//============================================
	// IAT_DR_RadialMenuDisplay
	//============================================
	...
	// Gestures
	//============================================
	protected void RefreshRadialOptions( IAT_DR_RadialMenuCategories category_id = -1 )
	{
		...
		{
			GetGestureItems( m_RadialItems, IAT_DR_RadialMenuCategories.CATEGORIES );
			m_CurrentCategory = -1;
	...
	protected void GetGestureItems( out array<ref IAT_DR_RadialMenuItem> gesture_items, IAT_DR_RadialMenuCategories category)
	{
		gesture_items.Clear();
		...
		if (GetDayZGame())
		{
			...
			{
				if ( category == IAT_DR_RadialMenuCategories.CATEGORIES )
				{
					int rangeCategoryCounter = 0;
					int minRange = iat_DR_config.GetMinRange();
					int maxRange = iat_DR_config.GetMaxRange();
					string categoryText = "";
					while(minRange <= maxRange)
					{
						categoryText = string.Format("%1km", ((rangeCategoryCounter * 1) + 1));
						gesture_items.Insert( new IAT_DR_RadialMenuItem(rangeCategoryCounter, categoryText, category ) );

						minRange += 1000;
						rangeCategoryCounter++;
					}
				}
				else
				{
					int categoryMax = ((category * 1000) + 1000);
					int stepInterval = 50;
					if (categoryMax > 5000)
						stepInterval = 100;

					int rangeCounterMax = (1000 / stepInterval);
					int rangeStep = 0;
					// category starting at 0 represents 50-1000m increments
					for (int rangeIndex = 0; rangeIndex < rangeCounterMax; rangeIndex++)
					{
						rangeStep = (category * 1000) + (rangeIndex * stepInterval) + stepInterval;
						gesture_items.Insert(new IAT_DR_RadialMenuItem( rangeIndex, string.Format("%1m", rangeStep), category));
					}
				}
			}
		}
	...
	protected void UpdateQuickbarItemCard( IAT_DR_RadialMenuItem iat_radialmenu_item )
	{
	...
	protected void ExecuteSelectedCategory( Widget w )
	{
		...
		if ( w )
		{
			...
			//is category
			if ( !instance.m_IsCategorySelected && iat_radialmenu_item.GetCategory() == IAT_DR_RadialMenuCategories.CATEGORIES )
			{
		...
		if ( instance.m_IsCategorySelected && instance.m_SelectedItem )
		{
			...
			if ( !GetGame().IsDedicatedServer() )
			{
				...
				if ( iat_radialmenu_item )
				{
					...
					rpc.Send(player, IAT_DYNAMICRADIO_RPC.IAT_RPC_DR_RADIAL_MENU, true, player.GetIdentity());
					CloseMenu();
				}
			}
		}
	...
	//only moves to the IAT_DR_RadialMenuCategories.CATEGORIES for now
