class IAT_SC_RadialMenuDisplay extends UIScriptedMenu
{
	protected Widget 							m_ItemCardPanel;
	protected ref array<ref IAT_SC_RadialMenuItem> 	m_RadialItems;
	...
	//instance
	static IAT_SC_RadialMenuDisplay							instance;

	//============================================
	// IAT_SC_RadialMenuDisplay
	//============================================
	void IAT_SC_RadialMenuDisplay()
	{
		m_RadialItems = new array<ref IAT_SC_RadialMenuItem>;
		if ( !instance )
			instance = this;
	}
	...
	{
		g_Game.GetUIManager().EnterScriptedMenu( IAT_SC_MENU_RADIAL_MULTI, NULL );
	}
	...
	override void Update(float timeslice)
	{
		...
		if (m_CurrentCategory != IAT_SC_RadialMenuCategories.CATEGORIES && m_CurrentCategory != -1 && player)
		{
	...
	// Gestures
	//============================================
	protected void RefreshRadialOptions( IAT_SC_RadialMenuCategories category_id = -1 )
	{
		...
		{
			GetGestureItems( m_RadialItems, IAT_SC_RadialMenuCategories.CATEGORIES );
			m_CurrentCategory = -1;
	...
	protected void GetGestureItems( out array<ref IAT_SC_RadialMenuItem> gesture_items, IAT_SC_RadialMenuCategories category)
	{
		gesture_items.Clear();
		...
		if (category == IAT_SC_RadialMenuCategories.CATEGORIES)
		{
			gesture_items.Insert(new IAT_SC_RadialMenuItem( IAT_SC_RadialMenuCategories.LETTERS, "Letters", category));
			gesture_items.Insert(new IAT_SC_RadialMenuItem( IAT_SC_RadialMenuCategories.NUMBERS, "Numbers", category));
			gesture_items.Insert(new IAT_SC_RadialMenuItem( IAT_SC_RadialMenuCategories.SYMBOLS, "Symbols", category));
		}
		else if (category == IAT_SC_RadialMenuCategories.LETTERS)
		{
			// 26 letters to add
			for (int i = 0; i < 26; i++)
			{
				gesture_items.Insert(new IAT_SC_RadialMenuItem( i, MiscGameplayFunctions.IAT_GetCarvedSymbolName(i), category));
			}
		}
		else if (category == IAT_SC_RadialMenuCategories.NUMBERS)
		{
			// 10 numbers to add, starting after letters
			for (int j = 26; j < 36; j++)
			{
				gesture_items.Insert(new IAT_SC_RadialMenuItem( j, MiscGameplayFunctions.IAT_GetCarvedSymbolName(j), category));
			}
		}
		else if (category == IAT_SC_RadialMenuCategories.SYMBOLS)
		{
			int maxCount = MiscGameplayFunctions.IAT_GetCarvedSymbolNames().Count();
			// add the rest starting after 26 letters and 10 numbers
			for (int k = 37; k < maxCount; k++)
			{
				gesture_items.Insert(new IAT_SC_RadialMenuItem( k, MiscGameplayFunctions.IAT_GetCarvedSymbolName(k), category));
			}
		}
	}
	...
	protected void CreateGestureContent()
	{
		...
		for ( int i = 0; i < m_RadialItems.Count(); ++i )
		{
			IAT_SC_RadialMenuItem iat_radialmenu_item = m_RadialItems.Get( i );
	...
	protected void UpdateQuickbarItemCard( IAT_SC_RadialMenuItem iat_radialmenu_item )
	{
	...
	protected void MarkSelected( Widget w )
	{
		...
		if ( w )
		{
			IAT_SC_RadialMenuItem iat_radialmenu_item;
	...
	protected void ExecuteSelectedCategory( Widget w )
	{
		...
		if ( w )
		{
			...
			//is category
			if ( !instance.m_IsCategorySelected && iat_radialmenu_item.GetCategory() == IAT_SC_RadialMenuCategories.CATEGORIES )
			{
		...
		if ( instance.m_IsCategorySelected && instance.m_SelectedItem )
		{
			...
			if ( !g_Game.IsDedicatedServer() )
			{
				...
				if ( iat_radialmenu_item )
				{
					...
					rpc.Send(player, IAT_SIMPLECARPENTRY_RPC.IAT_RPC_SC_RADIAL_MENU, true, player.GetIdentity());
					// client side setting
					player.IAT_SetCarvedSymbolSelection(iat_radialmenu_item.GetID());
					CloseMenu();
				}
			}
		}
	...
	//only moves to the IAT_SC_RadialMenuCategories.CATEGORIES for now
