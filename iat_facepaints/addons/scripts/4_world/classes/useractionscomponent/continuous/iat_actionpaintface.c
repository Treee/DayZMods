class IAT_ActionPaintFaceCB extends ActionContinuousBaseCB
{
  	...
  	override void OnStateChange(int pOldState, int pCurrentState)
	{
		...
		{
			if (g_Game.GetUIManager() && g_Game.GetUIManager().IsMenuOpen(IAT_MENU_RADIAL_MULTI))
				g_Game.GetUIManager().FindMenu(IAT_MENU_RADIAL_MULTI).Close();
		}
		else if (pCurrentState == STATE_LOOP_LOOP && pOldState != STATE_LOOP_LOOP && (!g_Game.IsDedicatedServer()))
		{
      		g_Game.GetUIManager().EnterScriptedMenu(IAT_MENU_RADIAL_MULTI, g_Game.GetUIManager().GetMenu());
		}
	}
};
...
class IAT_ActionPaintFace extends ActionContinuousBase
{
  	...
  	override void OnUpdate(ActionData action_data)
	{
		...
		if(!g_Game.IsDedicatedServer())
			if (action_data.m_State == UA_FINISHED && g_Game.GetUIManager() && !g_Game.GetUIManager().IsMenuOpen(IAT_MENU_RADIAL_MULTI))
