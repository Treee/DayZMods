modded class ActionToggleTentOpen
{
	override bool ActionCondition( PlayerBase player, ActionTarget target, ItemBase item )
	{
		...
		{
			TentBase targetTent;
			// is the target a tent? (we know it is because super but we need the cast for later)
			if (Class.CastTo(targetTent, target.GetParent()))
			{
				if (targetTent.GetInventory())
				{
					IAT_Codelock_Colorbase iat_codelock;
					// is our codelock attached?
					if (Class.CastTo(iat_codelock, targetTent.GetInventory().FindAttachmentByName("Att_CombinationLock")))
					{
