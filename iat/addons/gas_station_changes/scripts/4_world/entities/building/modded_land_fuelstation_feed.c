	...
	override void EEKilled(Object killer)
	{
		...
		if (g_Game.IsDedicatedServer())
		{
			...
			if (Class.CastTo(rollConfig, GetDayZGame().GetIATRollTableConfig()))
			{
				...
				{
					potentialItem = rollConfig.GetRollTableResult("gas_station_explosion");
					rollConfig.SpawnRollTableResult(potentialItem, GetPositionWithinArea(33));
				}
			}
			#endif
			...
			// create a molotov
			if (Class.CastTo(entityResult, g_Game.CreateObjectEx("IAT_Molotov_Grenade_Incendiary", GetPositionWithinArea(), iFlags)))
			{
