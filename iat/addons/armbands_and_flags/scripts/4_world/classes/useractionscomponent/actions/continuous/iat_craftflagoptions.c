class IAT_FlagArmbandVariantActionReciveData extends ActionReciveData
{
  	int m_IATFlagArmbandVariant;
}

class IAT_FlagArmbandVariantActionData extends ActionData
{
  	int m_IATFlagArmbandVariant;
};


class IAT_ActionCraftFlagOptionsCB : ActionContinuousBaseCB
{
...
class IAT_ActionCraftFlagOptions: ActionContinuousBase
{
	void IAT_ActionCraftFlagOptions()
	{
		m_Sound = "craft_universal_0";
		m_CallbackClass = IAT_ActionCraftFlagOptionsCB;
	...
	{
		TStringArray m_FlagOptions = GetFlagOptions();
		if (m_FlagOptions.Count() > 0)
			GetVariantManager().SetActionVariantCount(m_FlagOptions.Count());
		else
	...
	{
		m_Text = "Craft Flag - No Option";
		Rag stackOfRags;
		if (target && Class.CastTo(stackOfRags, target.GetObject()))
		{
			if (stackOfRags.GetQuantity() == 6)
			{
				// if the main item sewing kit has enough quantity to sew a flag
				if (item && item.GetQuantity() > 24)
				{
					TStringArray m_FlagOptions = GetFlagOptions();
					if (m_FlagOptions.IsValidIndex(m_VariantID))
					{
						m_Text = string.Format("Craft Flag - %1", m_FlagOptions.Get(m_VariantID));
					}
				}
			}
		}
	...
	override void OnFinishProgressServer( ActionData action_data )
	{
		...
		if (action_data.m_Player)
		{
			...
			TStringArray m_FlagOptions = GetFlagOptions();
			if (m_FlagOptions.IsValidIndex(newItemIndex))
			{
				string newItem = string.Format("Flag_%1",  m_FlagOptions.Get(newItemIndex));
