...
class IAT_IncendiaryTrigger extends EffectTrigger
{
	const float DAMAGE_TICK_RATE = 2;//deal damage every n-th second

	override string GetDisplayName()
	{
		return "Incendiary Area";
	}
	...
	override void OnStayServerEvent(TriggerInsider insider, float deltaTime)
	{
		if ( m_DealDamageFlag )
		{
			...
			if(creature && creature.m_EffectTriggerCount != 0)
				creature.DecreaseHealth("", "", GameConstants.AI_INCENDIARY_DMG_PER_SEC * m_TimeAccuStay / creature.m_EffectTriggerCount);// we devide by m_EffectTriggerCount for multiple trigger presence(overlapping triggers)

			PlayerBase player;
			if (Class.CastTo(player, insider.GetObject()))
				player.DecreaseHealth("", "", GameConstants.AI_INCENDIARY_DMG_PER_SEC * m_TimeAccuStay);// we devide by m_EffectTriggerCount for multiple trigger presence(overlapping triggers)
		}
	}
...
class IAT_IncendiaryTrigger_Dynamic : IAT_IncendiaryTrigger
{
	protected int m_AreaState; // Used to get the state from linked area and update effects

	void IAT_IncendiaryTrigger_Dynamic()
	{
...
class IAT_IncendiaryTrigger_Local : IAT_IncendiaryTrigger
{
	override string GetAmbientSoundsetName()
	{
		return "";
